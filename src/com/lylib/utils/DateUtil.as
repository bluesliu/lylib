package com.lylib.utils
{
	public class DateUtil {
		...
		/**
		 * 将本地时间转换为通用时间（UTC），单位：秒
		 * @param date
		 * @return 
		 * 
		 */		
		public static function getTimestamp(date:Date):int
		{
			return Math.round(date.time/1000)-(date.timezoneOffset*60);
		}
		 ...
		 * 
		 */		
		public static function timestamp2Date(second:int):Date
		{
			var date:Date = new Date();
			date = new Date((second+date.timezoneOffset*60)*1000);
			return date;
		}
	}
}