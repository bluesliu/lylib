package com.lylib.air
{
	...
	public class NativeCommand extends EventDispatcher
	{
		...
		public function  readOutputMultiByte():String
		{
			return process.standardOutput.readMultiByte(process.standardOutput.bytesAvailable,"gb2312");
		}
		...
		public function dispose():void
		{
			if(_process)
			{
				_process.removeEventListener(Event.STANDARD_ERROR_CLOSE, this.process_standardErrorCloseHandler);
				_process.removeEventListener(Event.STANDARD_INPUT_CLOSE, this.process_standarInputCloseHandler);
				_process.removeEventListener(Event.STANDARD_OUTPUT_CLOSE, this.process_standardOutputCloseHandler);
				_process.removeEventListener(IOErrorEvent.STANDARD_ERROR_IO_ERROR, this.process_standardErrorIoErrorHandler);
				_process.removeEventListener(IOErrorEvent.STANDARD_INPUT_IO_ERROR, this.process_standardInputIoErrorHandler);
				_process.removeEventListener(IOErrorEvent.STANDARD_OUTPUT_IO_ERROR, this.process_standardOutputIoErrorHandler);
				_process.removeEventListener(NativeProcessExitEvent.EXIT, this.process_exitHandler);
				_process.removeEventListener(ProgressEvent.STANDARD_ERROR_DATA, this.process_standardErrorDataHandler);
				_process.removeEventListener(ProgressEvent.STANDARD_INPUT_PROGRESS, this.process_standardInputProcessHandler);
				_process.removeEventListener(ProgressEvent.STANDARD_OUTPUT_DATA, this.process_standardOutputDataHandler);
			}
			_process = null;
			_exeFile = null;
		}
		...
		protected function process_standardOutputDataHandler(event:ProgressEvent):void{
			...
			//trace("Got: ", process.standardOutput.readMultiByte(process.standardOutput.bytesAvailable,"gb2312"));
			dispatchEvent(event);
		}
